...
> Status: **built — v1 (keyword retrieval + Gemini/extractive answers).** Members-only chat at
...
- The outreach bot already uses **Gemini** for generation (`draft.py`
  personalization, `llm.py`) and already parses past projects (`past_projects.py`,
  `data/past_projects.json`). Reuse the Gemini key/pattern and the parsed corpus as a
  starting seed.
...
Query (portal):
  member question → embed → top-k similarity search (pgvector)
    → assemble context  → Gemini (answer + inline citations)  → stream to chat UI
```
...
- **LLM: Gemini** (already used by the bot) for synthesis; **embeddings** from a dedicated
...
3. **Retrieval API** — embed query → top-k search → context assembly → Gemini with strict
...
- **Cost**: embeddings (one-time-ish) + per-query Gemini calls — budget and cache.

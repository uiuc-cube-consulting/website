...
export async function getBoard(
  ...
  // The Drive folder link follows the round: a final-round candidate has a
  // first-round folder AND a final-round one, and the useful link is the one for
  // the round you are looking at. Selected per round rather than both at once
  // because `final_drive_folder_url` arrives with db/final-round-folders.sql,
  // which is applied by hand — asking for it unconditionally would take the whole
  // interview board down until that had happened. Spelled out rather than
  // interpolated because supabase-js parses the select list in the type system.
  const APPLICANT_COLUMNS =
    "id, name, email, year, major, college, stage, resume_file_id, resume_name, resume_mime, resume_match, resume_linked_at";
  // The two branches differ only in which folder-url column they carry, and
  // supabase-js types each one exactly; naming the union once keeps the rest of
  // the assembly reading a single shape.
  type ApplicantRow = {
    id: string;
    name: string;
    email: string;
    ...
    resume_linked_at: string | null;
    drive_folder_url?: string | null;
    final_drive_folder_url?: string | null;
  };
  const [applicantsRes, panelRes, reviewsRes, flagsRes] = await Promise.all([
    (round === "final_round"
      ? sb.from("applicants").select(`${APPLICANT_COLUMNS}, final_drive_folder_url`)
      : sb.from("applicants").select(`${APPLICANT_COLUMNS}, drive_folder_url`)
    )
  ...
  const visibleApplicants = excludeOwnApplications(
    viewer,
    (applicantsRes.data ?? []) as ApplicantRow[],
    (a) => a.email
  );
  ...
  const candidates: Candidate[] = visibleApplicants.map((a) => {
    ...
    return {
      ...
      driveFolderUrl:
        (round === "final_round" ? a.final_drive_folder_url : a.drive_folder_url) ?? null,

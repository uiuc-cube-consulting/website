...
type EmailTemplate = { subject: string; innerHtml: string };
...
<head>
  ...
  <style>
    ...
    h2 { font-size: 18px; font-weight: 700; color: #1a1a2e; margin: 0 0 16px; }
    p { font-size: 15px; line-height: 1.65; color: #555; margin: 0 0 16px; }
    blockquote { background: #f5f4f0; border-radius: 10px; padding: 16px 20px; margin: 20px 0; border: none; }
    blockquote p { margin: 4px 0; font-size: 14px; }
    blockquote strong { color: #1a1a2e; }
    ul, ol { font-size: 15px; line-height: 1.65; color: #555; margin: 0 0 16px; padding-left: 24px; }
    li { margin: 4px 0; }
...
/** Wrap editor inner HTML in the CUBE email shell. Used by API routes before sending via Resend. */
export function wrapInShell(subject: string, innerHtml: string): string {
...
export function approvalTemplate(
  ...
  const innerHtml = `<h1>Strike Notice: ${label}</h1>
<p>Hi ${memberName},</p>
...
export function requesterApprovalTemplate(
  ...
  const innerHtml = `<h1>Strike Request Approved</h1>
...
export function requesterDenialTemplate(
  ...
  const innerHtml = `<h1>Strike Request Denied</h1>
<p>Hi ${requesterName},</p>
...
export function execAlertTemplate(targetName: string): EmailTemplate {
  ...
  const innerHtml = `<h1>3-Strike Threshold Reached</h1>
